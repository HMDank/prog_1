...
void shuffle(char *a, int n) {
  ...
  for (int i = n - 1; i > 0; i--) {
    ...
    a[r] = a[i];
    a[i] = c;
    // shuffle from element n, n times
    // pick element n - 1, then switch the element with a
    // random element at index r (randomly taken)
  }
}

// Initializes the board with shuffled cards. Empty places are represented as '
// '. Open places are represented as the respective character, e.g., 'A'. Hidden
// places are represented as negative characters, e.g., -'A'. The cards are
// initially hidden (negative). Only initializes the elements of the cards
// array. Assumes that cards points to an existing array and that ncards, rows,
// cols, points, and turns are already set.
void init_cards(Board *b) {
  // b) todo: implement
  // access to board elements: b->ncards, b->cards[i], b->rows, b->cols
  for (int index = 0; index < b->ncards; index = index + 2) {
    b->cards[index] = -('A' + index / 2);
    b->cards[index + 1] = -('A' + index / 2);
  }

  for (int index = b->ncards; index < b->rows * b->cols; index++) {
    b->cards[index] = ' ';
  }
...
void print_board(Board *b) {
  // c) todo: implement
  // access to board elements: b->ncards, b->cards[i], b->rows, b->cols
  for (int row = 0; row <= b->rows + 1; row++) {
    if (row == 0) { // first row
      printf("  ");
      for (int temp_column = 1; temp_column <= b->cols; temp_column++) {
        printf("%d ", temp_column);
      }
    } else if (row == b->rows + 1) { // last row
      printf("\n%d points, %d turns\n", b->points, b->turns);
    } else {
      printf("\n");
      printf("%d ", row);
...
// Returns the array index of row r, column c. Stops the program if r or c are
// not valid.
...
void tests(void) {
  ...
  srand(123); // initialize random number generator to fixed seed to always get
              // the same test numbers
  init_cards(&b);
  ...
  print_board(
      &b); // ensure that output is equal to testsout2_want.txt or if you use
           // Windows that output is equal to testsout2_want_WIN.txt or if you
           // use Linux that output is equal to testsout2_want_LINUX.txt
...
int main(int argc, String argv[]) {
  ...
  Board b;
  b.rows = clamp(atoi(argv[1]), 2,
                 9); // atoi converts strings to integers (if possible)
