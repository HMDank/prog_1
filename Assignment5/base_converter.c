...
/*
    Gibt einen dynamisch allokierte String zurueck, der so viele Zeichen hat wie
   noetig sind um die Zahl in der Zielbasis darzustellen.
    @param number Zahl, die in der Zielbasis dargestellt wird.
    @param base Zielbasis, in der die Zahl number dargestellt werden soll.
    @return Eine Zeichenkette mit der noetigen Anzahl an Stellen mit '#'
   initialisiert.
*/
...
String convert_to_base(int number, int base) {
  int length = length_for_base(number, base);
  String placeholder = get_string_for_number_and_base(number, base);
  int temp = number;
  int rest;
  for (int i = 0; i < length; i++) {
    rest = temp % base;
    temp = (temp - rest) / base;
    s_set(placeholder, length - i - 1, characters[rest]);
  }
  return placeholder;
}

/*
    Schoen formatierte Ausgabe der Zahl, die zu konvertieren ist, sowie des
   Ergebnisses.
    @param number Zahl, die in der Zielbasis dargestellt wird.
    @param base Zielbasis, in der die Zahl number dargestellt werden soll.
*/
...
void test_convert_to_base() {
  ...
  print_convert(0, 2);
  test_equal_s(convert_to_base(0, 2), "0"); // Duplicated from line 71 (?)
...
void test_extract_bits() {
  ...
  test_equal_s(convert_to_base(extract_bits(0x1602, 0, 16), 2),
               "1011000000010");
