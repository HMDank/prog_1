...
void insert_into_directory(Node *directory, Node *new_node) {
  ...
  if (prev == NULL) {
    directory->dir.entries = new_entry(new_node, current);
  } else {
    prev->next = new_entry(new_node, current);
  }
}
...
void print_node(Node *node, char *prefix) {
  ...
  case NT_DIR:
    prefix_length =
        strlen(prefix) + strlen(node->name) + 2; // + slash + termination
...
Node *find_entry(Node *directory, char *name) {
  ...
  for (Entry *entry = directory->dir.entries; entry != NULL;
       entry = entry->next) {
