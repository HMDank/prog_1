...
Statistics compute_statistics(String table) {
  ...
  if (count > 0) {
    ...
    ps.mean_height_females =
        (females > 0) ? (sum_height_f / (double)females) : 0.0;
    ps.mean_height_diverse =
        (diverse > 0) ? (sum_height_d / (double)diverse) : 0.0;
    ps.mean_length_of_names = sum_name_lengths / (double)count;
  } else {
    ps.mean_year = 0;
    ps.mean_height_males = ps.mean_height_females = ps.mean_height_diverse =
        0.0;
